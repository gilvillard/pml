...
#if (1)
void MulMod_local_b(zz_pX& x, zz_pX& upper, const zz_pX& a, const zz_pXMultiplier& B, const zz_pXModulus& F)
{
    ...
    if (da >= n)
        LogicError(" bad args to MulMod_local_b(zz_pX, zz_pX, zz_pXMultiplier, zz_pXModulus)");
...
/*------------------------------------------------------------*/
void gen_pows_b (Vec<zz_pX> &pow, Vec<zz_pX>&upper,
               const zz_pX &t,
               const zz_pX &a,
               const long s,
               const zz_pX &g,
               const long l, const long need_upper_products = 0)
{
    ...
    if (need_upper_products)
    {
        ...
        for (long i = 1; i < l; i++)
        {
            MulMod_local_b(pow[i], upper[i - 1], pow[i - 1], a_pow_mul, g_mod);
            reverse(upper[i - 1], upper[i - 1], n - 2);
        }
        zz_pX tmp;
        MulMod_local_b(tmp, upper[l - 1], pow[l - 1], a_pow_mul, g_mod);
...
/*------------------------------------------------------------*/
void mul_special_b(Mat<zz_pX>& c, Mat<zz_pX>& cs, const Mat<zz_pX>& a, const Mat<zz_pX>& b)
{
    long dA = deg(a);
    long dB = deg(b);
    long sA = dA + 1;
               ...
               const long m)
{
    ...
    t = GetWallTime();
    Mat<zz_pX> res1, res2;
    mul_special_b(res1, res2, mat1, mat2);
...
/*------------------------------------------------------------*/
/*------------------------------------------------------------*/
void SetDims_b (Coeffs &res, const long r, const long c, const long d)
{
    res.SetLength(r);
    for (long i = 0; i < r; i++)
    {
...
/*------------------------------------------------------------*/
/*------------------------------------------------------------*/
void format_b (Vec<Mat<zz_p>> &res, const Vec<Coeffs> &coeffs, const long d, const long m)
{
    res.SetLength(d);
                    ...
                    const long m)
{
    ...
    Vec<zz_pX> upper;
    cout << "-enter pows\n";
    gen_pows_b(alphas, upper, t, a, 1, g, sqrt_d, 0);
    cout << "pow1: " << GetWallTime() - t1 << endl;
    t1 = GetWallTime();
    gen_pows_b(As, upper, zz_pX(1), a, sqrt_d, g, sqrt_d, 1);
                    ...
                    const long m)
{
    ...
    t1 = GetWallTime();
    SetDims_b(res, sqrt_d, sqrt_d, 2*m);
    zz_pX g_trunc;
    trunc(g_trunc,g,2*m);
    for (long u = 0; u < sqrt_d; u++)
        for (long v = 0; v < sqrt_d; v++)
                 ...
                 const long m)
{
    ...
    for (long i = 1; i < m; i++)
    {
        SetDims_b(res[i], sqrt_d, sqrt_d, 2 * m - i);
        for (long u = 0; u < sqrt_d; u++)
            for (long v = 0; v < sqrt_d; v++)
            {
    ...
    t = GetWallTime();
    for (long i = 0; i < m; i++)
        SetDims_b(res[i], sqrt_d, sqrt_d, m);
    format_b(mats, res, d, m);
